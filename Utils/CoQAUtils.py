...
# # POS tag set comes from the tagger (morphological tagset, not UPOS)
# POS = {w: i for i, w in enumerate([''] + list(nlp.get_pipe("tagger").labels))}

# # Entity label set comes from the NER pipe
# ENT = {w: i for i, w in enumerate([''] + list(nlp.get_pipe("ner").labels))}
...
#             datum['qas'] = [
#                 qa for qa in datum['qas']
#                 if len(qa['annotated_answer']['word']) == 1
...
#         if 'BERT' in self.opt:
#             x_bert_ids, x_bert_offsets = self.bertify(datum['annotated_context']['word'])
#             x_bert_mask = torch.LongTensor(1, len(x_bert_ids)).fill_(1)
#             x_bert = torch.tensor([x_bert_ids], dtype=torch.long)
...
#             # context features (first 4 dims always present)
...
#             # prepend a window of previous Q/A plus current Q
#             for j in range(i - self.prev_ans, i + 1):
#                 if j < 0:
#                     continue
#                 # during training, skip "unknown" questions (same as original logic)
#                 if not self.evaluation and datum['qas'][j]['answer_span'][0] == -1:
#                     continue

#                 # question tokens (+ special <Q>=2)
...
#                 # answer tokens (+ special <A>=3) from previous turns only
...
#                 ground_truth[i, 1] = 0
#                 answer_str = 'yes'
#             elif answer.lower() in ['no', 'no.']:
#                 ground_truth[i, 0] = 0
#                 ground_truth[i, 1] = -1
#                 answer_str = 'no'
#             elif answer.lower() in ['unknown', 'unknown.']:  # FIX: membership test (not == list)
#                 ground_truth[i, 0] = -1
#                 ground_truth[i, 1] = -1
#                 answer_str = 'unknown'
#             else:
...
#         if 'BERT' in self.opt:
#             bert_len = max(len(s) for s in q_bert_list) if q_bert_list else 0
...
#             # move to device (no Variables, no async cuda)
#             x_bert = x_bert.to(device, non_blocking=True)
#             x_bert_mask = x_bert_mask.to(device, non_blocking=True)
#             x_bert_offsets = x_bert_offsets.to(device, non_blocking=True)  # wasn’t moved before
#             query_bert = query_bert.to(device, non_blocking=True)
#             query_bert_mask = query_bert_mask.to(device, non_blocking=True)
#             query_bert_offsets = query_bert_offsets.to(device, non_blocking=True)
#         else:
#             x_bert = None
#             x_bert_mask = None
#             x_bert_offsets = None
...
#             x_char = x_char.to(device, non_blocking=True)
#             x_char_mask = x_char_mask.to(device, non_blocking=True)
#             query_char = query_char.to(device, non_blocking=True)
#             query_char_mask = query_char_mask.to(device, non_blocking=True)
#         else:
#             x_char = None
#             x_char_mask = None
#             query_char = None
...
#         x_ent = x_ent.to(device, non_blocking=True)
...
#         yield (
#             x, x_mask, x_char, x_char_mask, x_features, x_pos, x_ent,
#             x_bert, x_bert_mask, x_bert_offsets,
#             query, query_mask, query_char, query_char_mask,
#             query_bert, query_bert_mask, query_bert_offsets,
#             ground_truth, context_str, context_words, context_word_offsets,
#             answer_strs, context_id, turn_ids
#         )
...
from Models.Bert.tokenization import BertTokenizer
from Utils.GeneralUtils import normalize_text, nlp  # nlp is loaded & has sentencizer in GeneralUtils
from Utils.Constants import *
...
# -----------------------------
# spaCy v3-compatible POS / ENT
# -----------------------------
try:
    _tagger = nlp.get_pipe("tagger")
    POS = {w: i for i, w in enumerate([""] + list(_tagger.labels))}
except Exception:
    POS = {"": 0}

try:
    _ner = nlp.get_pipe("ner")
    ENT = {w: i for i, w in enumerate([""] + list(_ner.labels))}
except Exception:
    ENT = {"": 0}
...
# -----------------------------
def feature_gen(context, question):
    """Generate per-token features for context based on overlap with question."""
    counter_ = Counter(w.text.lower() for w in context)
    total = sum(counter_.values()) or 1
    term_freq = [counter_[w.text.lower()] / total for w in context]

    q_word = {w.text for w in question}
    q_lower = {w.text.lower() for w in question}
    q_lemma = {w.lemma_ if w.lemma_ != "-PRON-" else w.text.lower() for w in question}

    match_origin = [w.text in q_word for w in context]
    match_lower = [w.text.lower() in q_lower for w in context]
    match_lemma = [(w.lemma_ if w.lemma_ != "-PRON-" else w.text.lower()) in q_lemma for w in context]
...
def gen_upper_triangle(score_s, score_e, max_len, use_cuda):
    """
    Build upper-triangular span scores from start/end scores.
    score_s/e: (batch, context_len)
    returns: (batch, context_len * context_len)
    """
    batch_size, context_len = score_s.shape
    # (B, L, L)
    expand_score = score_s.unsqueeze(2).expand(batch_size, context_len, context_len) + \
                   score_e.unsqueeze(1).expand(batch_size, context_len, context_len)

    score_mask = torch.ones(context_len, device=score_s.device)
    tri = torch.outer(score_mask, score_mask).triu().tril(max_len - 1)
    empty_mask = tri.eq(0).unsqueeze(0).expand_as(expand_score)
    expand_score = expand_score.masked_fill(empty_mask, -float("inf"))
...
def _looks_like_path(p: str) -> bool:
    return (os.sep in p) or ("/" in p) or p.endswith(".txt") or p.endswith(".json")
...
class BatchGen:
    def __init__(self, opt, data, use_cuda, vocab, char_vocab, evaluation=False):
        ...
        self.prev_ans = self.opt.get("PREV_ANS", 2)
        self.prev_ques = self.opt.get("PREV_QUES", 0)
        self.use_char_cnn = "CHAR_CNN" in self.opt

        # Tokenizer (HF id or local vocab path) — don't prefix HF ids with datadir
        self.bert_tokenizer = None
        if "BERT" in self.opt:
            if "BERT_LARGE" in opt:
                print("Using BERT Large tokenizer")
                tok_spec = opt.get("BERT_large_tokenizer_file", "bert-large-uncased").strip().strip('"').strip("'")
            else:
                print("Using BERT base tokenizer")
                tok_spec = opt.get("BERT_tokenizer_file", "bert-base-cased").strip().strip('"').strip("'")

            if _looks_like_path(tok_spec):
                tok_resolved = os.path.abspath(os.path.join(opt["datadir"], tok_spec))
            else:
                tok_resolved = tok_spec  # HF repo id
            print("Loading tokenizer from:", tok_resolved)
            self.bert_tokenizer = BertTokenizer.from_pretrained(tok_resolved)
    ...
    def bertify(self, words):
        ...
        for word in words:
            ...
            x_bert_offsets.append([len(bpe), len(bpe) + len(sub)])
            bpe.extend(sub)
        bpe.append("[SEP]")
        ...
        for datum in data:
            ...
            for i in range(qa_len):
                ...
                for j in range(i - self.prev_ans, i + 1):
                    if j < 0:
                        continue
                    # during training, skip "unknown" questions
                    if not self.evaluation and datum["qas"][j]["answer_span"][0] == -1:
                        continue
                ...
                if answer.lower() in ["yes", "yes."]:
                    ...
                    ground_truth[i, 1] = -1
                    answer_str = "no"
                elif answer.lower() in ["unknown", "unknown."]:
                    ground_truth[i, 0] = -1
                    ground_truth[i, 1] = -1
                    answer_str = "unknown"
                else:
            ...
            if "BERT" in self.opt:
                ...
                x_bert = x_bert.to(device, non_blocking=True)
                x_bert_mask = x_bert_mask.to(device, non_blocking=True)
                x_bert_offsets = x_bert_offsets.to(device, non_blocking=True)
                query_bert = query_bert.to(device, non_blocking=True)
                query_bert_mask = query_bert_mask.to(device, non_blocking=True)
                query_bert_offsets = query_bert_offsets.to(device, non_blocking=True)
            else:
            ...
            if self.use_char_cnn:
                x_char_mask = (x_char != 0)
                query_char_mask = (query_char != 0)
                x_char = x_char.to(device, non_blocking=True)
                x_char_mask = x_char_mask.to(device, non_blocking=True)
                query_char = query_char.to(device, non_blocking=True)
                query_char_mask = query_char_mask.to(device, non_blocking=True)
...
def ensemble_predict(pred_list, score_list, voteByCnt=False):
    ...
    for phrases, scores in zip(pred_by_examples, score_by_examples):
        ...
        firstappear = defaultdict(int)
        for phrase, phrase_score, index in zip(phrases, scores, range(len(scores))):
            d[phrase] += 1.0 if voteByCnt else phrase_score
            if phrase not in firstappear:
                firstappear[phrase] = -index
        predictions += [max(d.items(), key=lambda pair: (pair[1], firstappear[pair[0]]))[0]]
        best_scores += [max(d.items(), key=lambda pair: (pair[1], firstappear[pair[0]]))[1]]
    return predictions, best_scores
...
def score(pred, truth, final_json):
    ...
    f1 = 100.0 * f1 / total
    no_f1 = 0.0 if no_total == 0 else 100.0 * no_f1 / no_total
    yes_f1 = 0.0 if yes_total == 0 else 100.0 * yes_f1 / yes_total
    no_ans_f1 = 0.0 if no_ans_total == 0 else 100.0 * no_ans_f1 / no_ans_total
