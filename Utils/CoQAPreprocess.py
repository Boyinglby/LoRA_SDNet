...
class CoQAPreprocess():
    def __init__(self, opt):
        ...
        self.test_file  = os.path.join(opt['datadir'], opt['CoQA_TEST_FILE']) if 'CoQA_TEST_FILE' in opt else None
        ...
        dataset_labels = ['train', 'dev'] + (['test'] if self.test_file else [])
        allExist = True
    ...
    def preprocess(self, dataset_label):
        ...
        elif dataset_label == 'dev':
            file_name = self.dev_file
        elif dataset_label == 'test':
            if not self.test_file:
                raise ValueError("CoQA_TEST_FILE not set in conf but 'test' requested.")
            file_name = self.test_file
        else:
            raise ValueError(f"Unknown dataset_label: {dataset_label}")
        ...
        with open(file_name, 'r') as f:
            dataset = json.load(f)
            # # a subset TO BE DELETED LATER
            # MAX_DIALOGS = int(self.opt.get("DEBUG_MAX_DIALOGS", 200)) #TO BE DELETED LATER
            # if MAX_DIALOGS > 0 and len(dataset["data"]) > MAX_DIALOGS:#TO BE DELETED LATER
            #     dataset["data"] = dataset["data"][:MAX_DIALOGS]#TO BE DELETED LATER
        ...
        if dataset_label == 'train':
            ...
            with open(meta_file_name, 'wb') as f:
                msgpack.dump(meta, f, use_bin_type=True)

        dataset['data'] = data

        # Write preprocessed JSON for train/dev/test
