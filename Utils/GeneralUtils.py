...
# keep tagger + ner (you use POS/ENT), disable parser for speed
nlp = spacy.load("en_core_web_sm", disable=["textcat", "lemmatizer", "attribute_ruler", "parser"])

# add a rule-based sentence segmenter so `doc.sents` works
...
def make_variable(x: torch.Tensor, use_cuda: bool):
    """
    Move a tensor to the right device and mark it as not requiring gradients.
    """
    device = torch.device("cuda" if use_cuda and torch.cuda.is_available() else "cpu")
    return x.to(device, non_blocking=True).detach()
