...
def seq_dropout(x, p: float = 0.0, training: bool = False):
    """
    Sequence dropout.
    x: (batch, seq_len, input_size)
    Drops whole features across the sequence (same mask across timesteps).
    """
    if (not training) or p <= 0.0:
        return x

    # Create dropout mask on same device/dtype as x
    keep_prob = 1.0 - p
    mask_shape = (x.size(0), x.size(2))  # (batch, input_size)
    dropout_mask = torch.bernoulli(torch.full(mask_shape, keep_prob, device=x.device, dtype=x.dtype))

    # Scale mask so expectation is preserved
    dropout_mask = dropout_mask / keep_prob

    return dropout_mask.unsqueeze(1) * x  # (batch, 1, input_size) -> broadcast over seq_len
...
class Attention(nn.Module):
    ...
    def forward(self, x1, x2, x2_mask, x3 = None, drop_diagonal=False):
        ...
        # ensure boolean mask and no .data
        if drop_diagonal:
            # scores: (batch, L, L) — mask out self-attention positions
            L = scores.size(1)
            diag_mask = torch.eye(L, dtype=torch.bool, device=scores.device).unsqueeze(0).expand_as(scores)
...
def generate_mask(new_data: torch.Tensor, dropout_p: float = 0.0) -> torch.Tensor:
    """
    new_data: a tensor whose shape we use to generate a mask
    dropout_p: dropout probability
    returns: mask tensor with the same shape as new_data
    """
    keep_prob = 1.0 - dropout_p
    # Start with all "keep_prob"
    base = torch.full_like(new_data, keep_prob)

    # Force at least one element to 1 per row
    for i in range(base.size(0)):
        one = random.randint(0, base.size(1) - 1)
        base[i, one] = 1.0

    # Bernoulli sample
    mask = torch.bernoulli(base)

    # Scale so expectation is preserved
    mask = mask / keep_prob
    return mask
