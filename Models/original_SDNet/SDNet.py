...
class SDNet(nn.Module):
    def __init__(self, opt, word_embedding):
        ...
        if 'BERT' in self.opt:
            ...
            if 'BERT_LINEAR_COMBINE' in self.opt:
                ...
                self.alphaBERT = nn.Parameter(torch.ones(bert_layers), requires_grad=True)
                self.gammaBERT = nn.Parameter(torch.ones(1, 1), requires_grad=True)
    ...
    def linear_sum(self, output, alpha, gamma):
        alpha_softmax = F.softmax(alpha, dim=0)  # specify dim explicitly
        for i in range(len(output)):

            # make BERT features plain constants so grads flow only to alpha/gamma
            t = output[i].detach() * alpha_softmax[i] * gamma
            if i == 0:
                res = t
            else:
                res += t
