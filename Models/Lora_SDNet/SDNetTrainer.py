...
class SDNetTrainer(BaseTrainer):
    ...
    def setup_model(self, vocab_embedding):
        # --- ensure logging target exists when running evaluate-only ---
        if not hasattr(self, "saveFolder") or self.saveFolder is None:
            # put eval artifacts under the feature cache by default
            base = self.opt.get("FEATURE_FOLDER", self.opt["datadir"])
            self.saveFolder = os.path.join(base, "eval")
            os.makedirs(self.saveFolder, exist_ok=True)
        if "logFile" not in self.opt:
            self.opt["logFile"] = "eval.log"
        # ---------------------------------------------------------------
        ...
        # ====== Inspect trainable params (LoRA should appear here if enabled) ======
        net_for_inspect = self.network  # no DDP wrapping here
        trainable = [(n, p) for n, p in net_for_inspect.named_parameters() if p.requires_grad]
        total_trainable = sum(p.numel() for _, p in trainable)
        print(f"[Sanity] trainable parameters: {total_trainable:,}")
        for n, _ in trainable[:30]:
            print("  +", n)
        # ===========================================================================

        # ====== Optional: split LoRA vs non-LoRA for different learning rates ======
        lora_params = [p for n, p in trainable if "lora" in n.lower()]
        base_params = [p for n, p in trainable if "lora" not in n.lower()]

        base_lr = float(self.opt.get("BASE_LR", 1e-3))
        lora_lr = float(self.opt.get("LORA_LR", 5e-4))
        weight_decay = float(self.opt.get("WEIGHT_DECAY", 0.0))
    ...
    def predict(self, batch):
        ...
        (x, x_mask, x_char, x_char_mask, x_features, x_pos, x_ent,
         ...
         answers, context_id, turn_ids) = batch
        ...
        score_s, score_e, score_yes, score_no, score_no_answer = self.network(
            ...
            x_bert, x_bert_mask, x_bert_offsets,
            query, query_mask, query_char, query_char_mask,
            query_bert, query_bert_mask, query_bert_offsets, len(context_words)
        )
        ...
        prob = torch.softmax(scores, dim=1).detach().cpu()

        predictions, confidence, pred_json = [], [], []

        for i in range(batch_size):
            # Top-1 id
            ids = torch.argsort(prob[i], descending=True)
            best_id = int(ids[0].item())
            confidence.append(float(prob[i, best_id].item()))

            if best_id < context_len * context_len:
                # map flat idx -> (st_word, ed_word)
                st_word = best_id // context_len
                ed_word = best_id % context_len
                # convert to char offsets (Python ints)
                st_char = context_word_offsets[int(st_word)][0]
                ed_char = context_word_offsets[int(ed_word)][1]
                predictions.append(context_str[st_char:ed_char])
            elif best_id == context_len * context_len:
                predictions.append('no')
            elif best_id == context_len * context_len + 1:
                predictions.append('yes')
            elif best_id == context_len * context_len + 2:
                predictions.append('unknown')
            else:
                # fallback (shouldn't happen)
                predictions.append('')

            # turn_ids might be tensor; make it an int
            turn_id_val = int(turn_ids[i].item() if isinstance(turn_ids[i], torch.Tensor) else turn_ids[i])
    ...
    def save_for_predict(self, filename, epoch):
        # Start from the full state dict (minus large embeddings caches)
        network_state = dict([(k, v) for k, v in self.network.state_dict().items()
                              if k[0:4] != 'CoVe' and k[0:4] != 'ELMo' and k[0:9] != 'AllenELMo'])
        ...
        # === KEEP LoRA adapter weights in Bert; drop the huge frozen base ===
        if 'BERT_LORA' in self.opt:
            for k in list(network_state.keys()):
                if k.startswith('Bert') and ('lora' not in k.lower()):
                    del network_state[k]
        else:
            # Original behavior: drop all Bert weights when not using LoRA
            for k in list(network_state.keys()):
    ...
    def evaluate_preprocessed(self, model_path: str, which: str = "dev"):
        """
        Evaluate a saved checkpoint on a *preprocessed* CoQA file.
        `which` is 'dev' or 'train' or 'test' (if you've preprocessed a test split).
        Looks for:  <FEATURE_FOLDER>/coqa-{which}-preprocessed.json
        """
        # Build the model (same as in train())
        self.vocab, self.char_vocab, vocab_embedding = self.preproc.load_data()
        self.setup_model(vocab_embedding)
        self.network.eval()
        self.network.drop_emb = False
        ...
        pre_path = os.path.join(self.opt['FEATURE_FOLDER'], f"{self.data_prefix}{which}-preprocessed.json")
        with open(pre_path, "r", encoding="utf-8") as f:
            data = json.load(f)

        # We need predictions + gold answers to score
        batches = BatchGen(self.opt, data["data"], self.use_cuda, self.vocab, self.char_vocab, evaluation=True)

        predictions, confidence, final_json, dev_answer = [], [], [], []
        for dev_batch in batches:
            phrase, phrase_score, pred_json = self.predict(dev_batch)
            predictions.extend(phrase)
            confidence.extend(phrase_score)
            final_json.extend(pred_json)
            dev_answer.extend(dev_batch[-3])  # ground-truth answer strings from BatchGen

        result, all_f1s = score(predictions, dev_answer, final_json)

        # Save artifacts next to your run dir or feature dir
        save_dir = getattr(self, "saveFolder", self.opt.get("FEATURE_FOLDER", self.opt["datadir"]))
        os.makedirs(save_dir, exist_ok=True)

        with open(os.path.join(save_dir, f"{which}_predictions.json"), "w") as out:
            json.dump(final_json, out)

        with open(os.path.join(save_dir, f"{which}_score_per_instance.json"), "w") as out:
            json.dump([{"id": j["id"], "turn_id": j["turn_id"], "f1": s}
                       for j, s in zip(final_json, all_f1s)], out)

        print(f"[Eval-{which}] F1={result['f1']:.2f}  (normal {result['normal_f1']:.2f} | yes {result['yes_f1']:.2f} | no {result['no_f1']:.2f} | unknown {result['no_ans_f1']:.2f})")
        return result
